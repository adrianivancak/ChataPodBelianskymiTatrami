...
const PAGE_SLUGS = { home: '', calendar: 'kalendar', gallery: 'galeria', nearby: 'okolie', contact: 'kontakt', reviews: 'recenzie' };
const SLUG_TO_PAGE = Object.fromEntries(Object.entries(PAGE_SLUGS).map(([p, s]) => [s, p]));

let SITE = { heroTitle:'', heroTagline:'', aboutText:'', basePrice:120, maxGuests:4, capacityNote:'', minNights:3, maxNights:null, bookingHorizonDays:365, altitude:'896 m', trailTime:'3 min', address:'Ždiar, Belianske Tatry, Slovensko' };
let AMENITIES = [];
let FACTS = [];
let HIGHLIGHTS = [];
...
function parseLocation(){
  ...
  const slug = parts[0] || '';
  return SLUG_TO_PAGE[slug] !== undefined ? SLUG_TO_PAGE[slug] : 'home';
}
...
function updateMeta(){
  const titles = {
    ...
    calendar: 'Kalendár — Chata pod Belianskymi Tatrami',
...
function renderSpecialOffers(){
  ...
  wrap.innerHTML = SPECIAL_OFFERS.map(o => `
    <div class="special-offer special-offer--${o.color || 'amber'}">
      ...
      ${o.text ? `<p>${o.text}</p>` : ''}
      ${o.ctaEnabled !== false ? `<button class="btn-primary special-offer-cta" onclick="navigate('calendar')">${o.ctaLabel || 'Rezervovať'}</button>` : ''}
    </div>
  `).join('');
}
...
async function checkAdminSession(){
  try{ const res = await api('GET', '/api/admin/check'); if(res.isAdmin){ showAdminMain(); } }catch(e){}
}
async function tryAdminLogin(){
  const val = document.getElementById('adminPass').value;
  try{ await api('POST', '/api/admin/login', {password: val}); showAdminMain(); }
...
async function populateAdminForms(){
  ...
  renderVouchersAdmin(); renderGalleryAdmin(); renderNearbyAdmin(); renderDealsAdmin();
