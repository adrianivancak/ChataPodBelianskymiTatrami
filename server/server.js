...
const DEFAULT_DATA = {
  ...
  stayRules: [],
  gallery: [
    { id: "g1", url: "", caption: "Chata v zime", tone: "#4A5D53" },
    { id: "g2", url: "", caption: "Obývačka s krbom", tone: "#8B6A45" },
    { id: "g3", url: "", caption: "Terasa s výhľadom na Havran", tone: "#C99A44" },
    { id: "g4", url: "", caption: "Výhľad na Ždiarsku Vidlu", tone: "#5B6F5C" },
    { id: "g5", url: "", caption: "Spálňa v podkroví", tone: "#6B4F32" }
  ],
  deals: [],
  pricing: [],
  nearby: [
...
app.put('/api/admin/content', requireAdmin, (req, res) => {
  const data = loadData();
  const { heroTitle, heroTagline, aboutText, basePrice, maxGuests, capacityNote, minNights, maxNights, bookingHorizonDays, address } = req.body;
  data.site = {
...
app.post('/api/admin/gallery', requireAdmin, (req, res) => {
  ...
  const data = loadData();
  const item = { id: 'g_' + Date.now(), url, caption: caption || '' };
...
app.post('/api/admin/gallery/upload', requireAdmin, upload.single('photo'), (req, res) => {
  ...
  const item = { id: 'g_' + Date.now(), url: `/images/uploads/${req.file.filename}`, caption: req.body.caption || baseName };
  data.gallery.push(item);
  saveData(data);
  res.json({ ok: true, item });
});
...
// ---------------------------------------------------------------------------
const PAGE_SLUGS = ['kalendar', 'galeria', 'okolie', 'kontakt', 'recenzie'];
app.get(['/', ...PAGE_SLUGS.map(s => '/' + s)], (req, res) => { res.sendFile(path.join(PUBLIC_DIR, 'index.html')); });
